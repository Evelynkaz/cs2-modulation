...
2. **Карты** — «Подготовить» по карте запускает цепочку: извлечение
   коллизий → точки стояния → радар → 3D-модель (всё кэшируется, прогресс и
   отмена прямо в списке). Новую карту можно извлечь по имени (`de_mirage`).
3. **Экран карты** — 2D-радар и 3D-вид (переключатель). Цель — точка
   (клик) или область падения («граната упала где угодно в области»);
   откуда бросать — вся карта, спавны, точка (правый клик) или обведённая
   область, при многоэтажности — выбор этажа/высоты. Дальше граната,
   типы бросков, клики, допуск — и поиск: прогресс идёт вживую, найденные
   раскидки выдаются списком с `setpos …; setang …`, траекторией и
   стабильностью.
...
2D-радар и текстурированную 3D-карту с запечённым освещением и 3D-скайбоксом,
области «откуда бросать» и области цели, а также `cs2mod export-glb <map>`
(экспортирует 3D-модель в кэш карты; обычно запускается кнопкой
«Подготовить»).
